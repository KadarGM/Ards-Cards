...
func _process(delta):
	if GameGlobal.is_searching == false:
		release_searching_hand()
	if is_on_button == false:
		if Input.is_action_just_pressed("right click"):
			GameGlobal.is_searching = true
			release_detail_card()
...
func release_detail_card():
	...
	tween.tween_property(self,"rotation", rot, .3)
	set_stats_visible(false)
	set_play_menu(false)
	top_level = false
...
func _on_use_card_toggled(toggled_on):
	if toggled_on:
		detail_card()
		GameGlobal.is_searching = false
	if not toggled_on:
		release_detail_card()
		GameGlobal.is_searching = true

func _on_active_card_mouse_entered():
	if GameGlobal.is_searching == true:
		searching_hand()
