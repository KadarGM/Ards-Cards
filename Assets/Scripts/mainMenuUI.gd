...
func animation(type,dir1,dir2,cond):
	...
	tween.tween_property(type, "position", Vector2(dir1,dir2), 0.7)
	if cond == false:
...
func start():
	...
	logo_rect.scale *= logo.size.x/1000
	logo_rect.position = Vector2(logo.size.x + 1000,(logo.size.y - logo_rect.size.y)/2)
...
func _on_play_button_pressed():
	animation(logo_rect,logo.size.x + 1000,(logo.size.y - logo_rect.size.y)/2,false)
